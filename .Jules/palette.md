...
## 2024-05-20 - Inline Form Validation Over Native Alerts
**Learning:** Found a native `alert()` being used for form validation when submitting a 'tree' sized task without subtasks. Native alerts are disruptive, modal, and often lack context for screen reader users, breaking the flow. Using inline error states with `role="alert"` and clearing the error on user interaction provides a much smoother, accessible experience.
