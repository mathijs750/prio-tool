...
## 2024-06-01 - Accessible Form Validation
**Learning:** Using native `alert()` for form validation is disruptive and poorly supported by screen readers. Furthermore, error messages should be localized to match the UI language.
**Action:** Prefer inline error states with `role="alert"` over native `alert()` when validating form submissions, especially inside modals or dialogs, to prevent disruptive behavior and maintain accessibility context for screen readers. Ensure all error messages match the Dutch localization of the app.
