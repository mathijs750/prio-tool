...
## 2024-06-26 - Keyboard Navigation Focus States
**Learning:** Found that interactive elements like buttons and icons lacked clear visual focus indicators when navigating via keyboard (tabbing). This violates accessibility standards, making it difficult for keyboard-reliant users to track their current position on the page.
**Action:** Always add strictly scoped `:focus-visible` styles (e.g., `outline: 2px solid var(--accent); outline-offset: 2px;`) to interactive component classes in CSS to ensure keyboard focus is visually apparent without relying on global selectors.
