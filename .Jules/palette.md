...
## 2024-06-07 - Inline Error States in Modals
**Learning:** Using native `alert()` for form validation inside `<dialog>` modals creates a disruptive UX and causes screen readers to lose context. It also ignores the application's localization.
**Action:** Prefer inline error states with `role="alert"` over native `alert()` when validating form submissions, especially inside modals or dialogs, to prevent disruptive behavior and maintain accessibility context for screen readers. Ensure error messages match the application's localization (e.g., Dutch).
