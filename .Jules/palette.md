...
## 2024-05-19 - Inline Validation over Native Alerts
**Learning:** Found that `AddTaskModal` used a native `alert()` to enforce the requirement of having at least one subtask for "tree" sized tasks. Native alerts are disruptive to the user experience and can cause users using screen readers to lose context. Replacing them with an inline error state that utilizes `role="alert"` maintains the user's focus and context, providing a smoother, more accessible form validation experience.
**Action:** Always prefer inline error states with `role="alert"` over native `alert()` calls for form validation, especially inside dialogs and modals, to ensure an accessible and non-disruptive user experience.
