...
## 2026-05-23 - Inline Validation over Native Alert
**Learning:** Replacing native `alert()` with inline `role="alert"` for form validation inside modals prevents disruptive browser behavior and maintains the accessibility context for screen reader users without losing focus.
**Action:** Prefer inline error states with `role="alert"` over native `alert()` when validating form submissions, particularly within modals.
