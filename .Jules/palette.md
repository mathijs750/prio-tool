...
## 2024-06-28 - Inline Validation Errors
**Learning:** Using native `alert()` for form validation within modals creates a disruptive experience and loses context for screen reader users.
**Action:** Prefer inline error messages using `role="alert"` to maintain design consistency and context. Always ensure error states are cleared upon user interaction (like `onChange` or `onClick`) to avoid stale messages.
