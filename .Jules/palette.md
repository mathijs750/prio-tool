...
## 2024-05-20 - Inline Form Validation
**Learning:** Using native `alert()` for form validation inside modals is disruptive and lacks accessibility context for screen readers. Replacing it with an inline error state using `role="alert"` improves the user experience and accessibility.
**Action:** Always prefer inline error states over native `alert()` when validating form submissions, especially within dialogs.
