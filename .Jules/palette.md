...
**Learning:** Using native `alert()` for form validation inside modals disrupts the user experience, blocks the main thread, and causes screen readers to lose context. Inline error messages with `role="alert"` provide a much smoother, accessible experience that stays within the app's UI context.
