...
**Learning:** Using a native `alert()` inside a native `<dialog>` breaks context for screen readers and creates a disruptive UX. Users lose their focus position in the form.
**Action:** Prefer inline error states with `role="alert"` positioned near the relevant inputs or submit button, using localized text.
