...
## 2024-05-18 - Inline Validation Errors
**Learning:** Using native `alert()` for form validation inside a modal is disruptive to the user flow and poses accessibility challenges. Inline error messages with `role="alert"` provide immediate, contextual feedback without breaking the user out of the application interface.
**Action:** Prefer inline error states with `role="alert"` over native `alert()` when validating form submissions, especially inside modals or dialogs. Ensure these error states are cleared when the user interacts with the relevant form fields again.
