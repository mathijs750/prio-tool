...
## 2026-07-08 - Keyboard Navigation Focus Indicators
**Learning:** Found that custom interactive components across the application lacked explicit keyboard focus indicators, making keyboard navigation difficult to track. Using `focus-visible` ensures these indicators only appear for keyboard users, preventing visual clutter for mouse users while significantly improving accessibility.
**Action:** Always provide explicitly scoped `:focus-visible` styles (e.g., `outline: 2px solid var(--accent); outline-offset: 2px;`) for custom interactive elements in `index.css` to ensure they are accessible via keyboard navigation.
