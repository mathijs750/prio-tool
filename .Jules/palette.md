...
## 2024-06-25 - Inline Form Validation
**Learning:** Using native `alert()` for form validation inside a modal is disruptive and lacks accessibility context. Replacing it with inline error states using `role="alert"` provides a much smoother experience and properly announces errors to screen readers without breaking the user out of the modal context.
**Action:** Prefer inline error states with `role="alert"` over native `alert()` when validating form submissions, especially inside modals or dialogs. Ensure error states are cleared upon user interaction.
