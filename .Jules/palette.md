...
## 2024-06-17 - Inline Form Validation Alerts
**Learning:** Found that `AddTaskModal` used a native `alert()` for form validation (requiring sub-tasks for 'tree' size tasks). Native alerts are disruptive, lose accessibility context for screen readers, and block the UI.
**Action:** Replace native `alert()` calls in form submissions with inline error messages using `role="alert"` and inline styling matching the design system (e.g., `var(--prio-high)`).
