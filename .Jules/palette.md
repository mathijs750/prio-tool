...
## 2024-05-24 - Inline Validation Errors
**Learning:** Found a native `alert()` being used for form validation in `AddTaskModal` (preventing "tree" size tasks without sub-tasks). Native alerts are disruptive, block the main thread, and lack proper accessibility context for screen readers when they pop up over modals.
**Action:** Always prefer inline error states with `role="alert"` for form validation within modals or dialogs to prevent disruptive behavior and maintain a smooth user experience.
