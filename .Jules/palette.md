...
## 2024-05-19 - Keyboard Accessibility Focus
**Learning:** Relied on `outline: none` for visual polish, making keyboard navigation difficult as focus indicators were absent. Adding `:focus-visible` ensures focus rings only appear for keyboard users (not mouse clicks), balancing visual design with critical accessibility requirements.
**Action:** Always include `:focus-visible` styles with sufficient contrast (like `outline: 2px solid var(--accent)`) globally and on custom inputs that strip default outlines.
