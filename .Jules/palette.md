...
## 2024-05-19 - Native Alerts in Dialogs
**Learning:** Found that form validation inside native `<dialog>` modals used native `alert()`, which causes disruptive behavior and loses context for screen readers.
**Action:** Prefer inline error states with `role="alert"` over native `alert()` when validating form submissions inside modals or dialogs.
