...
## 2024-05-19 - Form Validation UX
**Learning:** Found that `AddTaskModal` used a native `alert()` for form validation (e.g., when a "tree" task was submitted without subtasks). Native alerts are disruptive, can block the main thread, and often cause screen readers to lose context. Preferring inline error states with `role="alert"` provides immediate, context-aware feedback that is screen-reader friendly and less jarring.
