...
## 2024-06-29 - Inline Form Validation
**Learning:** Using native `alert()` for form validation inside modals disrupts the user flow and loses accessibility context. Screen readers handle inline `role="alert"` messages much better, providing immediate context without modal hijacking.
**Action:** Always prefer inline error messages with `role="alert"` styled with existing color tokens over native browser alerts for form validation. Ensure the error state is cleared upon subsequent user interaction.
