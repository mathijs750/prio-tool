...
## 2024-05-19 - Inline Validation Error States
**Learning:** Found that the `AddTaskModal` component used a native `alert()` when validating that a 'tree' size task requires at least one subtask. This is disruptive and provides poor context for assistive technologies. Replacing it with an inline error message with `role="alert"` maintains focus within the modal and clearly associates the error with the form context.
**Action:** Prefer inline error states with `role="alert"` over native `alert()` when validating form submissions, especially inside modals or dialogs, to prevent disruptive behavior and maintain accessibility context.
