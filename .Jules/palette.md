...
## 2024-06-24 - Inline Form Validation Errors in Modals
**Learning:** Using native `alert()` for form validation inside a modal dialog creates a disruptive, inaccessible experience for screen reader users and can cause focus management issues.
**Action:** Always prefer inline error states with `role="alert"` over native `alert()`. Ensure the error state is cleared upon subsequent user interaction with the form. Use inline styles leveraging existing CSS variables (e.g., `var(--prio-high)`) when necessary to avoid adding new custom CSS classes.
