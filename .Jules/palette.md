...
**Learning:** Using native `alert()` for form validation inside native `<dialog>` elements creates a disruptive experience, especially for screen reader users who lose context.
**Action:** Always prefer inline error messages with `role="alert"` over native `alert()` when validating form submissions in dialogs, ensuring they are localized in Dutch.
