...
## 2024-06-20 - Inline Validation Errors in Modals
**Learning:** Native `alert()` calls for validation inside native `<dialog>` elements create disruptive, inaccessible experiences and break the user's flow. Moving to an inline error state with `role="alert"` using existing CSS variables keeps context and improves screen reader support. It's crucial to clear this state dynamically (e.g., `onChange`, `onClick`) so users aren't looking at stale errors while fixing their input.
**Action:** Always prefer inline `role="alert"` messages over `alert()` for form validation, ensuring the error state is cleared upon subsequent user interaction.
