...
export function AddTaskModal({ isOpen, description, onClose, onSubmit }: IAddTaskModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (size === 'tree' && filteredSubTasks.length === 0) {
      setError("Voor een 'tree' taak is minimaal één deeltaak vereist.");
      return;
    }
      ...
      <form onSubmit={handleSubmit}>
        ...
        {showSubTasks && (
          <div className="form-group">
            ...
            {error && <div className="error-message" role="alert" style={{ color: 'var(--prio-high)', fontSize: '0.88rem', marginBottom: '0.5rem', display: 'block' }}>{error}</div>}
