...
export function AddTaskModal({ isOpen, description, onClose, onSubmit }: IAddTaskModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (size === 'tree' && filteredSubTasks.length === 0) {
      setError('Voor een grote taak (boom) is minimaal één deeltaak vereist.');
      return;
    }
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        ...
        {error && (
          <div role="alert" style={{ color: 'var(--prio-high)', fontSize: '0.88rem', marginBottom: '1rem', fontWeight: 'bold' }}>
            {error}
          </div>
        )}
