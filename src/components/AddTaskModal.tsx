...
export function AddTaskModal({ isOpen, description, onClose, onSubmit }: IAddTaskModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (size === 'tree' && filteredSubTasks.length === 0) {
      setError('Een "tree" taak vereist minimaal één deeltaak.');
      return;
    }
      ...
      <form onSubmit={handleSubmit}>
        ...
        {error && (
          <div role="alert" style={{ color: 'var(--prio-high)', marginBottom: 'var(--spacing)', fontWeight: 'var(--font-weight-bold)' }}>
            <span className="material-icons" style={{ verticalAlign: 'middle', marginRight: '0.25rem' }} aria-hidden="true">error_outline</span>
            {error}
          </div>
        )}
